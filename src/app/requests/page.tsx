...
export default function RequestsPage() {
  ...
  function Header({ title: heading, subheading }: { title: string; subheading: string }) {
    return (
    ...
    return (
      <ScrollArea className="max-w-full flex-1 min-h-0">
        <div className="flex gap-4 pb-2 pr-2 px-4 h-full">
          {statuses.map((col) => (
            <div key={col.key} className="min-w-64 flex-1 h-full flex flex-col rounded-md border border-gray-200 bg-gray-50">
              {/* <div className="text-sm font-medium mb-2">{col.title}</div> */}
              <Header title={col.title} subheading={`${items.filter((r) => r.status === col.key).length} item(s)`} />
