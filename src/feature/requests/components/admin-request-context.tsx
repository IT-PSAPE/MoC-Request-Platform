...
import { RequestTable, MembersTable } from "@/shared/database";
...
const AdminRequestContext = createContext<AdminRequestContextType | null>(null);

export function useAdminRequestContext() {
    const context = useContext(AdminRequestContext);

    if (!context) throw new Error("useAdminRequestContext must be used within a AdminRequestContextProvider");

    return context;
}

export function AdminRequestContextProvider({ children }: { children: React.ReactNode }) {
        ...
        const loadDefaults = async () => {
            try {
                ...
                if (membersResult.error) {
                    console.error("Failed to load statuses", membersResult.error);
                } else {
    ...
    const updateRequestStatusOptimistic = async (requestId: string, newStatusId: string) => {
        ...
        if (error) {
            ...
            // Revert the optimistic update by reverting the status
            setRequests((prevRequests) =>
                prevRequests.map((request) =>
                    request.id === requestId ? { ...request, status: oldStatus } : request
                )
            );
            throw error;
        }
    ...
    return (
        <AdminRequestContext.Provider value={context}>
            {children}
